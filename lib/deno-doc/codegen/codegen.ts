...
const typesDtsURL =
  "https://github.com/denoland/deno_doc/raw/4a8ba25b5dc0347fd3c182226e88de487920cbc6/js/types.d.ts";
...
function createWalkFile(
  ...
  sourceFile.addImportDeclaration({
    isTypeOnly: true,
    namedImports: [symbol],
    moduleSpecifier: "@deno/doc/types",
  });
...
function parseTypeReferenceNodes(docNodes: DocNode[]): TypeReferenceNodeMap {
  const referenceNodes = docNodes.reduce((result, docNode) => {
    ...
    return result.set(docNode.name, [
      ...(result.get(docNode.name) ?? []),
      typeReferenceNode,
    ]);
  ...
  referenceNodes.forEach((nodes, key) => {
    for (const node of nodes) {
      node.properties = node.properties?.filter((property) =>
        referenceNodes.has(property.typeName)
      );
      node.typeNames = node.typeNames?.filter((typeName) =>
        referenceNodes.has(typeName)
      );
      node.kinds = node.typeNames?.map((typeName) => {
        ...
        if (currentNode.kind === "interface") {
          const kind = currentNode.interfaceDef.properties.find(
        ...
        if (
          ...
          return currentNode.typeAliasDef.tsType.union.map((tsTypeDef) => {
            ...
            const currentCurrentNode = findDocNode(docNodes, {
            ...
            return currentCurrentNode.interfaceDef.properties.find(
              (property) => property.name === "kind",
            )?.tsType?.repr!;
          });
        }
